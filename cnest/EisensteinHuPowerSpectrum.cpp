...
/* The function returns growth_cb */
{
    ...
    return tf_cbnu ; // CHANGED HERE TO tf_cbnu FROM tf_cb
}
...
void LoadEinseinsteinHuWithMassiveNu(const ParametersCosmology & cosmo, const redshift & z0, ParamsP11 & params, const size_t & Nmax, const double & kmin, const double & kmax) {
  // Set f
  params.f = LinearGrowthRate (cosmo,z0) ;
  ...
  TFmdm_set_cosm(Omega_m, Omega_b, Omega_nu, N_nu, 1.-Omega_m, h, z0) ;
  
  double k[Nmax], Plin[Nmax] ;
  for (unsigned int i = 0 ; i < Nmax ; i++) {
    k[i] = kmin * exp(i*dk) ;
    TFmdm_onek_hmpc(k[i]) ;
    Plin[i] = 2.*M_PI*M_PI * As * k[i] / pow(100/299792.458,4) * pow(k[i]/kstar, ns-1) * pow(tf_cbnu*growth_to_z0, 2) ;
  }
